...
Global Const $CNG_VERSION = "1.9.2"
...
Func __CryptoNG_BcryptInitAuthModeInfo($tAuthInfoStruct)
	...
	If $__gbDebugging Then _DebugOut(@CRLF & "Function: __CryptoNG_BcryptInitAuthModeInfo()")
...
Func __CryptoNG_IsKeyBitLengthValid($hAlgorithmProvider, $vEncryptionKey)
	...
	If $__gbDebugging Then _DebugOut("dwMinLength = " & $iMinBitLength)
	If $__gbDebugging Then _DebugOut("dwMaxLength = " & $iMaxBitLength)
	If $__gbDebugging Then _DebugOut("dwIncrement = " & $iBitIncrement)
	...
	Else
		...
		Next
	EndIf
	If $__gbDebugging Then _DebugReportVar("$aValidBitLengths", $aValidBitLengths)
	...
	If $__gbDebugging Then _DebugOut("$bReturnValue = " & $bReturnValue)
